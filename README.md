...
- “透明度”只影响悬浮卡片背景，文字和控件保持清晰，不是整个原生窗口统一设置透明度。
- 普通窗口场景下支持始终置顶；macOS 全屏应用和不同 Spaces 的覆盖行为仍受系统限制。
- 点击穿透开启后无法直接点击悬浮窗，需要使用 `⌘⇧L` 恢复。
- 当前自然语言日期解析不支持任意格式的数字日期，只支持上文列出的相对日期和星期表达。
